...
class mavudp(mavfile):
    ...
    def __init__(self, device, input=True, broadcast=False, source_system=255, source_component=0, use_native=default_native):
        ...
        if len(a) == 2:
            ...
            a = device.rsplit(':', 1)
            temp_addr = (a[0], int(a[1]), 0, 0)
            print("mavudp __init__ IPv6: ")
            print(temp_addr)
        else:
            print("Blake's mavudp bad parse")
            print("UDP ports must be specified as host:port")
            sys.exit(1)
        self.port = socket.socket(socketFamily, socket.SOCK_DGRAM)
        self.udp_server = input
        self.broadcast = False
        if input:
            self.port.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
            self.port.bind(temp_addr)
        else:
            self.destination_addr = temp_addr
            if broadcast:
